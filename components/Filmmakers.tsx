...
export default function Filmmakers() {
  ...
  //   FILMMAKERS 가 걷히며 FESTIVALS 로 넘어감 → 0.82 로 더 앞당겨, 이후 [0.82~1.0](≈0.18=약 94vh)
  //   구간엔 마지막 카드가 중앙에 "머무른"(pinned·클램프) 채 순수 FILMMAKERS 만 보이다가, 조금 더
  //   스크롤하면 FESTIVALS 가 드러남. (C0 는 0.35 유지 — 타이핑 게이트 0.24~0.3 이후 첫 카드 도착 순서 보존)
  const C_END = 0.82;
  // 중앙 sticky: 카드가 중앙에 오면 이 절반 폭(진행도)만큼 x 를 고정(플래토) → 스크롤해도 그 구간엔
  // 카드가 중앙에 "붙어" 머묾. 값↑ = 더 오래 고정(더 sticky). 카드 간격의 절반 미만이어야 함.
  // C_END 를 0.82 로 낮추면 카드 중앙 간격 = (0.82-0.35)/5 = 0.094 로 좁아짐 → 0.045 면 전환 구간이
  //   0.004 로 거의 점프(스프링이 못 쫓아옴). 0.03 으로 줄여 전환 구간을 0.094-0.06=0.034(≈18vh)로 넓힘.
  //   트레이드오프: 중간 카드 플래토가 0.09→0.06 으로 짧아져 "스냅감"이 살짝 약해지지만, 정작 중요한
  //   마지막 카드의 체류는 [0.82~1.0] 클램프 꼬리로 보장되므로 손해 없음. 스프링(무거운 관성)은 그대로 유지.
  const DWELL = 0.03; // 중앙 고정(플래토) 반폭 — C_END 하향으로 좁아진 간격에 맞춰 축소, 전환 구간 확보
