...
export default function Filmmakers() {
  ...
  // 마지막 카드(Yuki Tanaka)가 중앙에 오는 진행도. 1.0(섹션 끝)이면 마지막 카드 등장과 동시에
  //   FILMMAKERS 가 걷히며 FESTIVALS 로 넘어감. 0.82 는 꼬리(=[0.82~1.0]≈94vh)가 너무 길어 x 가
  //   클램프로 고정된 "죽은 스크롤"처럼 느껴짐 → 0.90 으로 올려 꼬리를 [0.90~1.0]=0.10×520vh≈52vh
  //   ("잠시 머무름")로 단축. 덤으로 카드 중앙 간격이 (0.90-0.35)/5=0.11 로 넓어져 전환 구간이 여유로워짐
  //   → 무거운 스프링이 잘 따라와 마지막 카드가 늦게 안착하던 현상도 완화. (0.90 초과는 정착시간이 다시
  //   꼬리를 잡아먹어 상한선.) (C0 0.35 유지 — 타이핑 게이트 0.24~0.3 이후 첫 카드 도착 순서 보존)
  const C_END = 0.9;
  // 중앙 sticky: 카드가 중앙에 오면 이 절반 폭(진행도)만큼 x 를 고정(플래토) → 스크롤해도 그 구간엔
  // 카드가 중앙에 "붙어" 머묾. 값↑ = 더 오래 고정(더 sticky). 카드 간격의 절반 미만이어야 함.
  // C_END 0.90 에서 카드 중앙 간격 = (0.90-0.35)/5 = 0.11 로 넓어짐 → 간격이 늘어난 만큼 DWELL 을
  //   0.03→0.035 로 소폭 키워 중앙 "스냅감"을 되살림. 그래도 전환 구간 = 0.11-0.07 = 0.04(≈21vh)로
  //   지난번(0.82/18vh)보다 넉넉해 무거운 스프링이 무리 없이 따라옴. 간격 절반(0.055) 미만이라 안전.
  const DWELL = 0.035; // 중앙 고정(플래토) 반폭 — 넓어진 간격에 맞춰 소폭 상향, 스냅감·전환 여유 양립
